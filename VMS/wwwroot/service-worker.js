// VMS PWA Service Worker (offline fallback + robust precache)
console.log("[SW] Boot v8");
/** @type {string} */
const CACHE_NAME = "vms-cache-v8";
...
self.addEventListener('install', (event) => {
    ...
    event.waitUntil((async () => {
        const cache = await caches.open(CACHE_NAME);           // 'vms-cache-v8'
        await cache.addAll(PRECACHE_URLS);                     // actually precache
    })());
});
    ...
    if (url.pathname === '/favicon.ico') {
        event.respondWith((async () => {
            try {
                ...
                const cache = await caches.open(CACHE_NAME); // 'v8' right now
